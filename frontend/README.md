# AutoSystem — front-end

Front-end para o sistema de gestão da concessionária, cobrindo o escopo da
...
As funções HTTP tipadas, uma por endpoint, estão em `src/api` (ver `src/api/README.md`, com o mapa método do store → endpoint). Todo acesso a dados passa por `src/data/store.ts`, que hoje guarda o estado em memória e
...
src/
  ...
  api/                cliente HTTP e funções por endpoint (docs/04)
