 ...
 * - JSON de ida e volta, datas em `yyyy-mm-dd`, dinheiro como string decimal.
...
export class ApiError extends Error {
  ...
  constructor(status: number, mensagem: string, campos: Record<string, string> = {}, dados?: unknown) {
    super(mensagem)
    this.name = "ApiError"
    this.status = status
    this.campos = campos
...
async function lerErro(resposta: Response): Promise<ApiError> {
  let corpo: { erro?: string; campos?: Record<string, string> } & Record<string, unknown> = {}
  try {
  ...
  void _e
  return new ApiError(resposta.status, mensagem, campos ?? {}, Object.keys(dados).length ? dados : undefined)
}

async function requisicao<T>(
  metodo: "GET" | "POST" | "PUT" | "PATCH" | "DELETE",
  caminho: string,
  opcoes: { corpo?: unknown; query?: Query; signal?: AbortSignal } = {}
): Promise<T> {
  let resposta: Response
  try {
    resposta = await fetch(`${BASE}${caminho}${montarQuery(opcoes.query)}`, {
      method: metodo,
      headers: {
