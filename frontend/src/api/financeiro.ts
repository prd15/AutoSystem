...
export const financeiroApi = {
  lancamentos: {
    ...
    criar: (dados: LancamentoEntrada) =>
      api.post<LancamentoDTO>("/lancamentos", dados),

    /** PUT /api/lancamentos/{id} — com venda_id, só status, forma e data são aceitos. */
    atualizar: (
      id: number,
      dados: Partial<LancamentoEntrada>
    ) =>
    ...
    quitar: (
      ...
      api.patch<LancamentoDTO>(`/lancamentos/${id}/quitar`, {
        forma,
      }),
  ...
  saidasPorCategoria: (
    ...
    api.get<SaidaPorCategoriaDTO[]>(
      "/financeiro/saidas-por-categoria",
      periodo,
      signal
    ),
  ...
  financiamentos: {
    ...
    criar: (dados: FinanciamentoEntrada) =>
      api.post<FinanciamentoDTO>(
        "/financiamentos",
        dados
      ),

    /** PATCH /api/financiamentos/{id}/parcela — incrementa parcelas_pagas. */
    registrarParcela: (id: number) =>
      api.patch<FinanciamentoDTO>(
        `/financiamentos/${id}/parcela`
      ),

    /** POST /api/financiamentos/simular — sem gravar. O front também sabe simular (lib/financeiro.ts). */
    simular: (dados: SimulacaoEntrada) =>
      api.post<SimulacaoDTO>(
        "/financiamentos/simular",
        dados
      ),
  },
}