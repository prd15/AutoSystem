...
import { ORDEM_STATUS, ROTULO_STATUS, store, type Status, type Veiculo } from "@/data/store"
...
const MARCAS_SUGERIDAS = [
  "Chevrolet", "Fiat", "Ford", "Honda", "Hyundai", "Jeep", "Nissan", "Peugeot", "Renault", "Toyota", "Volkswagen",
]
...
/** Validação do escopo: obrigatórios, ano entre 1950 e ano atual + 1, km ≥ 0, preço > 0, placa única. */
function validar(f: Form, idAtual?: number): Erros {
  const e: Erros = {}
  const anoMax = new Date().getFullYear() + 1

  if (!f.marca.trim()) e.marca = "Informe a marca."
  else if (f.marca.trim().length > 50) e.marca = "A marca deve ter até 50 caracteres."

  if (!f.modelo.trim()) e.modelo = "Informe o modelo."
  else if (f.modelo.trim().length > 50) e.modelo = "O modelo deve ter até 50 caracteres."

  const ano = Number(f.ano)
  if (!f.ano.trim()) e.ano = "Informe o ano."
  else if (!Number.isInteger(ano) || ano < 1950 || ano > anoMax)
    e.ano = `O ano deve estar entre 1950 e ${anoMax}.`

  if (!f.cor.trim()) e.cor = "Informe a cor."

  const km = paraNumero(f.quilometragem)
  if (f.quilometragem.trim() === "") e.quilometragem = "Informe a quilometragem."
  else if (km === null || km < 0 || !Number.isInteger(km))
    e.quilometragem = "A quilometragem deve ser um número inteiro igual ou maior que zero."

  const preco = paraNumero(f.preco)
  if (f.preco.trim() === "") e.preco = "Informe o preço."
  else if (preco === null || preco <= 0) e.preco = "O preço deve ser maior que zero."
...
/**
 * Cadastro e edição no mesmo formulário (requisito do escopo), em um painel
 * lateral que mantém a listagem visível ao fundo, como um sheet do macOS.
 */
export function VeiculoSheet({
  ...
  /** Status pré-selecionado ao criar a partir de uma coluna do quadro. */
  ...
  useEffect(() => {
    if (aberto) {
      setForm(veiculo ? deVeiculo(veiculo) : { ...vazio, status: statusInicial })
      setErros({})
      setTocado(false)
    }
  }, [aberto, veiculo, statusInicial])

  const campo = <K extends keyof Form>(k: K) => (valor: Form[K]) => {
    setForm((f) => {
      const novo = { ...f, [k]: valor }
      if (tocado) setErros(validar(novo, veiculo?.id))
      return novo
    })
  }
  ...
  const salvar = (e: React.FormEvent) => {
    ...
    if (editando) {
      ...
      toast.success("Veículo atualizado", { description: `${dados.marca} ${dados.modelo}` })
    } else {
      store.criarVeiculo(dados)
      toast.success("Veículo cadastrado", { description: `${dados.marca} ${dados.modelo} entrou no estoque.` })
    }
    onOpenChange(false)
  }
  ...
  return (
    <Sheet open={aberto} onOpenChange={onOpenChange}>
      <SheetContent className="glass shadow-pop flex w-full flex-col gap-0 border-l-0 p-0 sm:max-w-[480px]">
        <form onSubmit={salvar} className="flex h-full flex-col" noValidate>
          <SheetHeader className="border-b px-6 py-5">
            ...
            <SheetDescription className="text-[13px]">
              {editando
                ? `Alterando ${veiculo.marca} ${veiculo.modelo}. Qualquer campo pode mudar, inclusive o status.`
                : "Preencha os dados do veículo. Ele entra no estoque como disponível."}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="scroll-mac flex-1 space-y-5 overflow-y-auto px-6 py-5">
              ...
              <Field label="Ano" erro={erros.ano}>
                {(p) => (
                  <Input
                    ...
                    className="tabular"
                    value={form.ano}
                    onChange={(e) => campo("ano")(e.target.value.replace(/\D/g, "").slice(0, 4))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Quilometragem" erro={erros.quilometragem}>
                {(p) => (
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => campo("quilometragem")(e.target.value.replace(/\D/g, ""))}
                      placeholder="0"
                    />
                ...
                {(p) => (
                  <div className="relative">
                    ...
                    <Input
                      ...
                      onBlur={() => {
                        const n = paraNumero(form.preco)
                        if (n !== null) campo("preco")(moedaCampo(n))
                      }}
                      placeholder="0,00"
                    />
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Placa" erro={erros.placa} opcional dica="Única quando preenchida.">
                {(p) => (
                  <Input
                    {...p}
                    id="campo-placa"
                    ...
                    value={form.placa}
                    onChange={(e) =>
                      campo("placa")(e.target.value.replace(/[^A-Z0-9]/gi, "").toUpperCase().slice(0, 7))
                    }
                    placeholder="ABC1D23"
                  />
                )}
              ...
              <Field label="Status">
                {(p) => (
                  <Select value={form.status} onValueChange={(v) => campo("status")(v as Status)}>
                    <SelectTrigger id={p.id} className="w-full" aria-describedby={p["aria-describedby"]}>
                      <SelectValue />
                    </SelectTrigger>
