...
function nomeDoPerfil(perfil: PerfilUsuario) {
  switch (perfil) {
    case "admin":
...
export function AppShell({
  ...
  const iniciaisUsuario =
    iniciaisDoNome(nomeUsuario)

  const perfilUsuario = usuario
    ? nomeDoPerfil(usuario.perfil)
    : "Usuário"
  ...
  const contagens: Partial<
    Record<Rota, number>
  > = {
    estoque:
      ind.veiculosEmEstoque,
    vendas:
      ind.vendasNoMes,
    clientes:
      store.clientes().length,
    consorcios:
      store.consorcios().length,
    financiamentos:
      store.financiamentos()
        .length,
    financeiro:
      store.resumoFinanceiro()
        .pendentes,
  }
      ...
      >
        ...
        <nav className="scroll-mac flex flex-1 flex-col gap-4 overflow-y-auto px-3 pb-2">
          {GRUPOS.map((g) => {
            const rotasVisiveis =
              g.rotas.filter((r) =>
                rotasPermitidas.includes(r)
              )

            if (rotasVisiveis.length === 0) {
              return null
            }
